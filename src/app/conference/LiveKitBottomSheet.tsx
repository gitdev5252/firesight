...
const LiveKitBottomSheet: React.FC<LiveKitBottomSheetProps> = ({ open, onClose, meetingDuration,onInvite }) => {
    const {
        startScreenShare,
        stopScreenShare,
        isScreenSharing,
    ...
    return (
        <BottomSheet open={open} onClose={onClose}>
            <div className="justify-center items-center flex flex-col gap-3 p-4">
                <button
                    className={`backdrop-blur-[16px] text-[#FFFFFF] bg-[#080B1680] rounded-[15px] gap-4 flex flex-row items-center h-[44px] w-auto p-6 shadow-lg justify-center text-center flex-none cursor-pointer ${isScreenSharing ? 'border border-green-400' : ''}`}
                    onClick={() => {
                        console.log('rngg')
                        if (isScreenSharing) {
                            stopScreenShare();
            ...
            <div className="justify-center items-center flex  flex-row gap-3 ">
                ...
                <span className="text-[#FFFFFF] font-bold text-sm">{meetingDuration}</span>
            </div>
        </BottomSheet>
    );
};
