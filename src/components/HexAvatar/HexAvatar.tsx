...
export const HexAvatar: React.FC<HexAvatarProps> = ({
  ...
  // unique gradient ID to avoid collisions across instances/SSR
  const gid = useId();
  const gradientId = `hexGradient-${gid}`;
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden
      >
        {/* your original gradient-filled path with the same opacity */}
        <path
          ...
          fill={`url(#${gradientId})`}
          fillOpacity="0.4"   // keep your original softness
        />
        <path
          opacity="0.2"       // keep your original border softness
